pipeline {
    ...
    tools {
        nodejs "Node"  // Use Node tool configured in Jenkins (ensure it's Node 20.x)
    }
        ...
        stage('Run Tests') {
            steps {
                script {
                    ...
                    def testResult = 0
                    if (p == 'ios') {
                        testResult = sh(returnStatus: true, script: 'platformName=ios npx cucumber-js || exit 0')
                    } else if (p == 'android') {
                        testResult = sh(returnStatus: true, script: 'platformName=android npx cucumber-js || exit 0')
                    } else {
                        testResult = sh(returnStatus: true, script: 'npx cucumber-js || exit 0')
                    }
